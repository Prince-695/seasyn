...
export interface RelationalGridViewProps {
  table: TableSchema
...
export function RelationalGridView({
  table,
  queryResult,
  isLoading = false,
  onRefresh,
    ...
    <div className={cn("flex flex-col space-y-3", className)}>
      ...
      <div className="border-border/70 bg-card flex flex-wrap items-center justify-between gap-2.5 rounded-xl border p-3 shadow-xs">
        <div className="flex items-center gap-2">
          {/* Quick Search using shadcn Input */}
                    ...
                    >
                      ...
                      {columns.map((col) => {
                        ...
                        return (
                          <td
                            key={col.name}
                            onClick={() => setEditingCell({ row, column: col })}
